...
- [x] Auto Scaling(ECSサービス/ASG)の実装とスケーリングポリシー設計: `05-ecs-autoscaling`で完了(2026-07-23)。Target Tracking Scaling Policyの選定、ALBヘルスチェック設計込みで実装・実機確認・Terraform化まで実施
- [x] ALBのヘルスチェック設計、複数AZでの障害シミュレーション: ヘルスチェック設計は`05-ecs-autoscaling`で、複数AZでの障害シミュレーションは`06-multi-az-failure-simulation`で完了(2026-07-27)。ALB/ECSサービスの役割分担、ECSのAZ Rebalancing機能の理解まで到達した
...
- 2026-07-23: `05-ecs-autoscaling`完了、Auto Scaling関連の項目にチェックを入れた。`variable`/`locals`の使い分けはCURRENT_LEVEL.mdで格上げしたため以後の重点フォローから外す。「コピペ実装時の値の見直し漏れ」(4回目の再発)への対策として本人が休憩・ペース配分の工夫に合意したため、次回以降のセッション密度にも引き続き配慮する
- 2026-07-27: `06-multi-az-failure-simulation`完了、Phase 2の障害シミュレーション項目にチェックを入れた。EIP/プライベートIPの混同、コピペ実装時の見直し漏れは引き続き重点フォロー対象(いずれも再発しているが、指摘後の自己修正・自己診断の速さには進歩が見られる)。次点として「キャッシュ戦略(ElastiCache)、キューイング(SQS)」または「マルチリージョン・DR設計」が候補
